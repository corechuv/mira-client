...
export default function Catalog() {
  ...
  const crumbs: Crumb[] = [
    ...
    { label: t("breadcrumbs.catalog", "Каталог"), current: filterPath.length === 0 }
  ];
  ...
  return (
    <div className="container">
      ...
      <div className={styles.topbar}>
        <button className="btn btnGhost" onClick={() => setOpenSheet(true)}>
          {t("btn.filters", "Фильтры")}
        </button>
        <div className={styles.topbarRight}>
          <label className={styles.sortInline}>
            <select className="input" value={sort} onChange={(e) => setSort(e.target.value as any)}>
              <option value="popular">{t("sort.popular", "Сначала популярные")}</option>
              <option value="price-asc">{t("sort.priceAsc", "Сначала дешёвые")}</option>
              <option value="price-desc">{t("sort.priceDesc", "Сначала дорогие")}</option>
            </select>
          </label>
        </div>
      </div>
      <span className={styles.counter}>
        {t("catalog.found", "Найдено")}: <b>{products.length}</b>
      </span>
